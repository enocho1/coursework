...
for city in range(len(distance_matrix)):
    ...
    while len(tour)<len(distance_matrix):
        ...
        for i in range(len(distance_matrix)):
            if(distance_matrix[tour[-1]][i]<distance_matrix[tour[-1]][nearest]):
                if i not in tour:
                    nearest = i
        
        tour_length += distance_matrix[tour[-1]][nearest]
        tour.append(nearest)
    tour_length+=distance_matrix[tour[-1]][city]
    tours.append(tour)
    lengths.append(tour_length)
minimum = 0
for t in range(len(lengths)):
    if lengths[t]<lengths[minimum]:
        minimum = t
tour = tours[minimum]
tour_length = lengths[minimum]
"""
""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""
def cost(state,M):
    cost = 0
...
def mate(M,F):
    child = [M[0]]
    while(len(child)<len(M)):
        point = 0
        while F[point] !=child[-1]:
            point+=1
        if point == len(M)-1:
            point = -1
        x= random.random()
        mutate = True
        if x>=.5:
            if F[point-1] not in child:
                child.append(F[point-1])
                mutate = False
        else:
            if F[point+1] not in child:
                child.append(F[point+1])
                mutate = False
        if mutate:
            g= int(len(M)*random.random())
            t = 1
            while (F[g] in child) and(t):
                g = int(len(M)*random.random())
                t+=1
                if t>1000:
                    t=0
        ...
        if len(child)<len(M):
            ...
            mutate = True
            if x>=.5:
                if M[point-1] not in child:
                    child.append(M[point-1])
                    mutate = False
            else:
                if M[point+1] not in child:
                    child.append(M[point+1])
...
for r in range(50):
    random.shuffle(tour)
    tours.append(tour)
    lengths.append(cost(tour, distance_matrix))

n = int(len(distance_matrix)/70)+1
tours.append(tour)
...
for j in range(50):
    ...
    #for i in range(len(tours)):
        #print (tours[i])
     #   fitness.append(1/(lengths[i] +1))
    #x= max(fitness)
    for i in range(len(tours)):
        if random.random()> (lengths[i]/max(lengths)):
            X = mutate(tours[i],n)
            tours.append(X)
            lengths.append(cost(X, distance_matrix))

B = best(lengths,1)
tour = tours[B[0]]
tour_length = lengths[B[0]]
