...
def cost(state,M):
    ...
    for i in range(1,len(M)):
        cost+= M[state[i]][state[i-1]]
    cost+=M[state[0]][state[-1]]
    return cost

def mutate(l, n):
    for i in range(n):
        x = int(len(l)*random.random())
        y = int(len(l)*random.random())
        t = l[x]
        l[x] = l[y]
        l[y] = t
    return l
...
def nextgen(population,M, average):
    temp = []
    for tour in population:
        if cost(tour,M)<= average:
            temp.append(tour)
    population = temp
    n = len(population)
    ...
    fitness = []
    for tour in population:
        fitness.append(1/cost(tour,M)**2)
    S = sum(fitness)
    ...
    for i in range(len(fitness)):
        ...
        a = probsel(fitness)
        b = probsel(fitness)
        c = (cross(population[a],population[b]))
        d = (cross(population[b],population[a]))
        mut = random.random()
        if c not in gen2:
            if mut<0.1:
                gen2.append(mutate(c,1))
                gen2.append(mutate(d,1))
            else:
                gen2.append(c)
                #gen2.append(d)
            if mut >.98:
                gen2.append(mutate(c,int(len(c)/2)))
                gen2.append(mutate(d,int(len(d)/2)))
                gen2.append(population[a])
                gen2.append(population[b])
            
    gen2.append(population[best])
    return gen2
...
p = []
for d in M:
	p.append(statistics.mean(d))

T = statistics.mean(p)*len(M)
...
for i in range(100):
    ...
    while (len(population)<N):
        random.shuffle(basictour)
        t = copy.copy(basictour)
        c= cost(t,M)
        if c <= T:
            population.append(t)
            

    for Q in range(int(100)):
        costs = []
        for tour in population:
            costs.append(cost(tour,M))
        av = statistics.mean(costs)
        population = nextgen(population,M,av)
        BEST = min(costs)
print("population: ",len(population)," best: ",BEST)

for tours in population:
    if cost(tours,M) == min(costs):
        tour= tours
tour_length = cost(tour,M)
##for i in range(N):
##    tour.append(i)
##while (len(tours)<200):
##    random.shuffle(tour)
...
##
##
##for P in range(32000):
##    mini = min(lengths)
##    print(min(lengths), len(lengths))
##    fitness = []
##    weakness = []
##    new_gen = []
##    new_lens = []
...
##    for i in range(len(fitness)):
##        G = probsel(fitness)
##        new_gen.append(tours[G])
##        new_lens.append(lengths[G])
##        m = mutate(tours[G],var)
##        C = cost(m,M)
##        new_gen.append(m)
##        new_lens.append(C)
##    tours+= copy.copy(new_gen)
##    lengths += copy.copy(new_lens)
##        
